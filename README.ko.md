...
모터 노드가 실행 중이면 기본 5초 간격으로 각 모터의 읽기 전용 State 1(`0x9A`) 응답을 읽어 로그에 출력합니다. 노드 내부의 기존 RS485 lock 안에서 수행하므로, 같은 포트에 별도 진단 프로세스를 실행하면 안 됩니다.

```text
[right_arm] diagnostics(0x9A) 1:T=34C,V=24.04V,state=0x00,err=0x00,raw=22 64 09 00 00 00 00
```

- `T`: 모터 온도, `V`: 실측 모터 입력전압, `err`: 모터 오류 바이트입니다.
- `state`, `raw`는 펌웨어별 상태를 이후 오류 상황과 비교하기 위해 남기는 값입니다. 0이 아니더라도 그 자체로 오류라고 해석하지 않습니다.
- 이동 중 `err`가 0이 아니거나 전압이 비정상적으로 떨어지면 큰 이동을 멈추고, 재시도 전에 파워서플라이·전원 분배·배선·기계적 limit을 확인합니다. 노드는 모터 오류를 자동으로 지우지 않습니다.
- ROS 파라미터 `diagnostics_hz`로 진단 주기를 변경하며, `0`이면 이 polling을 끕니다.
